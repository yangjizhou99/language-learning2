-- 性能优化索引
-- 20250120000008_performance_indexes.sql
-- 注意：移除了 CONCURRENTLY 关键字以支持事务执行

-- Shadowing 相关索引优化
CREATE INDEX IF NOT EXISTS idx_shadowing_items_lang_level_created 
ON public.shadowing_items(lang, level, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_shadowing_items_lang_level_title 
ON public.shadowing_items(lang, level, title);

CREATE INDEX IF NOT EXISTS idx_shadowing_attempts_user_lang_created 
ON public.shadowing_attempts(user_id, lang, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_shadowing_attempts_item_user 
ON public.shadowing_attempts(item_id, user_id);

-- Cloze 相关索引优化
CREATE INDEX IF NOT EXISTS idx_cloze_items_lang_level_created 
ON public.cloze_items(lang, level, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_cloze_items_lang_level_title 
ON public.cloze_items(lang, level, title);

-- 词汇表索引优化
CREATE INDEX IF NOT EXISTS idx_vocab_entries_user_lang 
ON public.vocab_entries(user_id, lang);

CREATE INDEX IF NOT EXISTS idx_vocab_entries_term_lang 
ON public.vocab_entries(term, lang);

-- 全文搜索索引
CREATE INDEX IF NOT EXISTS idx_vocab_entries_term_gin 
ON public.vocab_entries USING gin(to_tsvector('simple', term));

CREATE INDEX IF NOT EXISTS idx_article_drafts_text_gin 
...
-- Alignment 相关索引
CREATE INDEX IF NOT EXISTS idx_alignment_packs_lang_status 
ON public.alignment_packs(lang, status);

CREATE INDEX IF NOT EXISTS idx_alignment_attempts_user_pack 
ON public.alignment_attempts(user_id, pack_id);

-- 会话相关索引
CREATE INDEX IF NOT EXISTS idx_shadowing_sessions_user_item 
ON public.shadowing_sessions(user_id, item_id);

CREATE INDEX IF NOT EXISTS idx_shadowing_sessions_created_at 
ON public.shadowing_sessions(created_at DESC);

-- 文章相关索引
CREATE INDEX IF NOT EXISTS idx_articles_lang_difficulty 
...
CREATE INDEX IF NOT EXISTS idx_cloze_drafts_status_lang_level 
ON public.cloze_drafts(status, lang, level);

CREATE INDEX IF NOT EXISTS idx_shadowing_drafts_status_lang_level 
ON public.shadowing_drafts(status, lang, level);

CREATE INDEX IF NOT EXISTS idx_alignment_packs_status_lang 
...
-- 统计查询优化索引
CREATE INDEX IF NOT EXISTS idx_shadowing_attempts_stats 
...
CREATE INDEX IF NOT EXISTS idx_cloze_items_cover 
