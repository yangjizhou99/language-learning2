...
const filteredItems = items.filter(
  (item) =>
    item.title.includes(searchQuery) && item.lang === selectedLang && item.level === selectedLevel,
);
...
// 验证连接
const { data, error } = await remoteSupabase.from('shadowing_items').select('id').limit(1);
```
...
await remoteSupabase.from('shadowing_items').upsert(items, {
...
for (let i = 0; i < items.length; i += batchSize) {
  ...
  await remoteSupabase.from('shadowing_items').upsert(batch);
}
```
...
```typescript
try {
  const { error } = await remoteSupabase.from('shadowing_items').upsert(items);
...
// 3. 测试连接
const { data, error } = await remoteSupabase.from('shadowing_items').select('count').limit(1);
