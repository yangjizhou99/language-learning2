...
export async function fetchFromWikipedia(
  ...
  const plain = await fetch(
    `https://${m[1]}.wikipedia.org/api/rest_v1/page/plain/${encodeURIComponent(title)}`,
  ).then((r) => r.text());
  return {
    lang,
    title,
...
export async function fetchFromWikinews(
  url: string,
  lang: FetchResult['lang'],
): Promise<FetchResult> {
  // 简化：抓 HTML → 去格式
...
export async function fetchFromGutenberg(
  url: string,
  lang: FetchResult['lang'],
): Promise<FetchResult> {
