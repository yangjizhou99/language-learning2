...
import type { Lexicon, SpanItem, PronounResolution, SVOTriple, ClozeItem } from "../../types/lexicon";
...
export function pass1(text: string, lang: Lang): SpanItem[] {
  ...
  for (const w of cons) {
    const re = new RegExp(`\\b${w.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`, lang==="en"?"gi":"g");
    let m; while ((m = re.exec(t))) items.push({ span: [m.index, m.index+m[0].length], tag: "connective", surface: m[0] });
  }
  // Time expressions: support regex patterns
  const times: string[] = LEX[lang].time || [];
  for (const p of times) {
    const re = new RegExp(p, "g");
    let m; while ((m = re.exec(t))) items.push({ span: [m.index, m.index+m[0].length], tag: "time", surface: m[0] });
  }
  return items;
}
...
export function pass2(text: string, lang: Lang): PronounResolution[] {
  const t = normalize(text);
  const sentences = t.split(SENT_SPLIT[lang]).map(s=>s.trim()).filter(Boolean);
  ...
  for (let i=0;i<sentences.length;i++){
    ...
    for (const p of pronList) {
      ...
      let m; while((m=re.exec(s))){
        ...
        const reNoun = lang==="en" ? /\b[A-Z][a-z]+|\b[a-z]{3,}\b/g : /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]{2,}/gu;
        let mm; while((mm=reNoun.exec(window))) {
...
export function pass3(text: string, lang: Lang): SVOTriple[] {
  const t = normalize(text);
  const sents = t.split(SENT_SPLIT[lang]).map(s=>s.trim()).filter(Boolean);
  ...
  for (const s of sents) {
    if (lang === "en") {
      ...
      const re = /\b([A-Za-z][A-Za-z\-']{1,})\b[^.!?]{0,40}?\b(is|are|was|were|be|become|makes|made|has|have|do|does|did|[a-z]+s|[a-z]+ed|[a-z]+ing)\b[^.!?]{0,40}?\b([A-Za-z][A-Za-z\-']{1,})\b/;
      const m = re.exec(s);
      if (m) triples.push(spanTriple(s, m, base));
    } else {
      // Subject-Verb-Object for Japanese/Chinese
      const re = /([\p{Letter}\p{Number}]{2,})[^。！？；]{0,12}?([是为成为属于包含导致表示决定促进提供采用包含])[^。！？；]{0,12}?([\p{Letter}\p{Number}]{2,})/u;
      const m = re.exec(s);
      if (m) triples.push(spanTriple(s, m, base));
    }
    base += s.length + 1;
...
export function makeCloze(text: string, lang: Lang, version: "short"|"long"): ClozeItem[] {
  ...
  for (const it of p1) candidates.push({ 
    start: it.span[0], 
    end: it.span[1], 
  ...
  if (lang === "en") {
    ...
    let m; while((m=re.exec(t))) {
      if (m[0].length >= 6) candidates.push({ 
        start: m.index, 
