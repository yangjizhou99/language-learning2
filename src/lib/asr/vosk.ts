...
interface VoskBrowserApi {
	createModel: (
		url: string,
		opts?: { progress_callback?: (info: { status?: string; loaded?: number; total?: number; file?: string }) => void }
	) => Promise<VoskModel>;
}
...
declare global {
	interface Window { Vosk?: VoskBrowserApi }
}
...
export async function warmUpVosk(modelUrl: string, onProgress?: (p: VoskProgress) => void): Promise<void> {
	...
	const model = await Vosk.createModel(modelUrl, onProgress ? { progress_callback: onProgress } : undefined);
...
async function decodeToFloat32Mono(blob: Blob): Promise<{ data: Float32Array; sampleRate: number }> {
