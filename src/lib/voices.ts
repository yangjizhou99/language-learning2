...
export type Voice = { name:string; type?:string; ssmlGender?:string; naturalSampleRateHertz?:number };
...
export async function getVoicesCached(lang: string, kind: Kind = "Neural2"): Promise<Voice[]> {
  ...
  const p = fetch(`/api/tts/voices?lang=${lang}&kind=${kind}`)
    .then(async r => {
      ...
      try { if (typeof window !== "undefined") localStorage.setItem(k, JSON.stringify({ t: Date.now(), v })); } catch {}
      return v as Voice[];
    })
    .catch(() => [] as Voice[])
