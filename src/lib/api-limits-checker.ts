...
export async function checkAPILimits(
  ...
  try {
    ...
    const dailyUsagePercent =
      limits.daily_calls_limit > 0 ? (usage.daily_calls / limits.daily_calls_limit) * 100 : 0;
    const monthlyUsagePercent =
      limits.monthly_calls_limit > 0 ? (usage.monthly_calls / limits.monthly_calls_limit) * 100 : 0;

    if (
      dailyUsagePercent >= limits.alert_threshold ||
      monthlyUsagePercent >= limits.alert_threshold
    ) {
      console.warn(
        `API usage approaching limit: Daily ${dailyUsagePercent.toFixed(1)}%, Monthly ${monthlyUsagePercent.toFixed(1)}%`,
      );
    }
...
export async function checkUserAIPermissions(
  ...
  try {
    ...
    if (modelPermissions.length > 0) {
      const modelPermission = modelPermissions.find(
        (mp: any) => mp.model_id === model && mp.provider === provider && mp.enabled,
      );
    ...
    if (modelPermissions.length > 0) {
      ...
      if (modelPermission) {
        ...
        if (usageError) {
          ...
          // 检查Token限制
          const dailyTokens =
            usageData?.reduce((sum, record) => sum + (record.tokens_used || 0), 0) || 0;
