...
export async function POST(req: NextRequest) {
  ...
  const conflictKeys = (body.conflictKeys?.trim() || '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  try {
    ...
    const readStream = (lc as any).query(
      copyTo(
        `COPY (select ${colList} from ${qTable(table)} ${whereClause}) TO STDOUT WITH (FORMAT csv, HEADER true)`,
      ),
    );
    ...
    if (mode === 'insert' || conflictKeys.length === 0) {
      ...
      await rc.query(
        `create temporary table ${qIdent(temp)} as table ${qTable(table)} with no data`,
      );
