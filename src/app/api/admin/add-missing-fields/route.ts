...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!modelPermsError) {
        // 使用 raw SQL 添加字段
        const { error: addModelPermsError } = await supabase.rpc('exec', {
    ...
    // 添加 api_keys 字段
    try {
      const { error: addApiKeysError } = await supabase.rpc('exec', {
    ...
    // 添加 ai_enabled 字段
    try {
      const { error: addAiEnabledError } = await supabase.rpc('exec', {
