...
export async function POST(req: NextRequest) {
  ...
  if (!lang||!genre||!difficulty||!title||!text) return NextResponse.json({ error:"缺字段" }, { status:400 });
  ...
  const p3c = p3.filter((t:any)=> sents.some(S=> t.s[0]>=S.start && t.o[1]<=S.end) && t.s[1]<=t.v[0] && t.v[1]<=t.o[0]);

  const { data, error } = await supabase.from("article_drafts").insert([{
    source:"manual", lang, genre, difficulty, title, text,
    license: license || "User-Provided", meta: { source_url },
    keys:{ pass1:p1c, pass2:p2c, pass3:p3c }, cloze_short: shortCloze, cloze_long: longCloze,
