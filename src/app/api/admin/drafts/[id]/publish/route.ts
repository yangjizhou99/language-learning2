...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id:string }> }) {
  ...
  if (d.status === "published" && d.published_article_id) return NextResponse.json({ ok:true, article_id:d.published_article_id });

  // 生成checksum
  const checksum = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(d.text))
    .then(hash => Array.from(new Uint8Array(hash)).map(b => b.toString(16).padStart(2, '0')).join(''));

  // 正式入库（事务性写入）
  const { data: art, error: e1 } = await supabase.from("articles").insert([{
    lang:d.lang, genre:d.genre, difficulty:d.difficulty, title:d.title, text:d.text,
    source_url: d?.meta?.source_url || null, license:d.license || null, 
    checksum,
  ...
  await supabase.from("article_drafts").update({ status:"published", published_article_id: art.id }).eq("id", id);
