...
export async function POST(req: NextRequest, { params }:{ params: Promise<{ id:string }> }) {
  ...
  if (!lang || !text || !keys) return NextResponse.json({ error:"missing fields" }, { status:400 });

  const len = text.length;
  const sents = splitSentencesWithIndex(text, lang);

  // Pass1：span 边界 & exact-match
  const p1 = (keys.pass1||[]).filter((k:any)=>
    inBounds(k.span, len) && exact(text, k.span, text.slice(k.span[0],k.span[1])) &&
    (k.tag==="connective"||k.tag==="time")
  );

  // Pass2：先行词必须在同句，且在代词之前
  const p2 = (keys.pass2||[]).map((x:any)=>({
    pron:x.pron,
    antecedents:(x.antecedents||[]).filter((a:Span)=>{
      const okBounds = inBounds(a,len) && inBounds(x.pron,len);
      if (!okBounds) return false;
      if (a[1] > x.pron[0]) return false;
      return sents.some(S => a[0]>=S.start&&a[1]<=S.end && x.pron[0]>=S.start&&x.pron[1]<=S.end);
    }).slice(-3)
  ...
  function cleanCloze(items:any[]){
    ...
    for (const it of items){
      ...
      out.push({ start:it.start,end:it.end,answer:ans,hint:it.hint||"blank",type:it.type||"collocation" });
      bySent.set(si,q+1);
    }
    return out;
  }
  ...
  return NextResponse.json({
    ...
    cloze_long: czL,
    report: {
      len, sentences: sents.length,
      pass1: p1.length, pass2: p2.length, pass3: p3.length,
      cloze_short: czS.length, cloze_long: czL.length
    }
  });
}
