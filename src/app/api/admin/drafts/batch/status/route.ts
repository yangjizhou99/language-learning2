...
export async function GET(req: NextRequest) {
	...
	const id = sp.get("id"); if (!id) return NextResponse.json({ error: "missing id" }, { status: 400 });

	const { data: batch, error } = await supabase.from("article_batches").select("*").eq("id", id).single();
	...
	(items || []).forEach((it: any) => {
		...
		perDifficulty[k] = perDifficulty[k] || { tasks: 0, prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 };
	...
	return NextResponse.json({
		ok: true,
		batch: {
			id: batch.id, name: batch.name, status: batch.status, provider: batch.provider, model: batch.model,
			lang: batch.lang, genre: batch.genre, words: batch.words, temperature: batch.temperature,
			totals: batch.totals, created_at: batch.created_at
		},
		counts,
		perDifficulty,
