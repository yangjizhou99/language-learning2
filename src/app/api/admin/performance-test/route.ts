  ...
  api: [
    {
      ...
      query: async () => {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/shadowing/next?lang=en&level=2`,
        );
        const duration = performance.now() - start;
    ...
    },
    {
      ...
      query: async () => {
        const start = performance.now();
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/cloze/next?lang=en&level=2`,
        );
        const duration = performance.now() - start;
