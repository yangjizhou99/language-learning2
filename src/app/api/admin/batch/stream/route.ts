...
const SYS_SHADOW =
  'You create concise shadowing scripts for language learners. Return VALID JSON only.';
...
function promptCloze(params: {
  lang: Lang;
  level: number;
  topic: string;
  blanksRange: [number, number];
  weights: { connector: number; collocation: number; grammar: number };
  batchSize: number;
}) {
...
function promptShadow({
  ...
  // 根据等级设置字数要求
  const wordCount =
    level <= 2 ? '30-50' : level === 3 ? '50-80' : level === 4 ? '80-120' : '120-180';
  const complexity =
    level <= 2 ? 'simple' : level === 3 ? 'intermediate' : level === 4 ? 'advanced' : 'expert';
...
async function runOne(
  kind: 'alignment' | 'cloze' | 'shadowing',
  task: Task,
  params: any,
  supabase: any,
  auth: any,
  ...
  if (kind === 'alignment') {
    ...
    sys = SYS_CLOZE;
    userPrompt = promptCloze({
      lang,
      level: task.level,
      topic: task.topic,
      blanksRange: range,
      weights: params.weights || { connector: 0.4, collocation: 0.3, grammar: 0.3 },
      batchSize,
    });
  } else {
    sys = SYS_SHADOW;
    userPrompt = promptShadow({
      lang,
      level: task.level,
      topic: task.topic,
      genre: params.genre || 'monologue',
      register: params.register || 'neutral',
      sentRange: params.sentRange || [6, 10],
      batchSize,
    });
  }
  ...
  if (kind === 'alignment') {
    ...
    const rows = items.map((item: any, index: number) => {
      ...
      if (passage.length < minLength) {
        ...
        throw new Error(
          `文本过短: 长度${passage.length}，等级${task.level}要求至少${minLength}字符`,
        );
      }
...
function estimateBlanks(lang: Lang, level: number) {
  ...
  const min = Math.max(
    3,
    Math.round(targetLen / (lang === 'en' ? 55 : 60)) + Math.floor(level / 2),
  );
  const max = min + 2;
...
export async function POST(req: NextRequest) {
  ...
  // 解析组合
  const topics: string[] = String(params.topicsText || '')
    .split('\n')
    .map((s: string) => s.trim())
    .filter(Boolean);
  const levels: number[] = Array.isArray(params.levels)
    ? params.levels.map((n: any) => Number(n))
      ...
      const workers = Array.from({ length: concurrency }, async () => {
        while (idx < tasks.length) {
          ...
          try {
            controller.enqueue(
              encoder.encode(
                sse({ type: 'progress', idx: i, topic: task.topic, level: task.level }),
              ),
            );
            ...
            const res: any = await withRetry(
              () =>
                runOne(
                  kind,
                  task,
                  { ...params, lang, provider, model, temperature, batchSize },
                  supabase,
                  auth,
                  i,
                ),
