...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  try {
    ...
    if (
      ...
      (!Array.isArray(allowed_languages) || !Array.isArray(allowed_levels))
    ) {
    ...
    const permissionsData = {
      user_id: userId,
      can_access_shadowing:
        can_access_shadowing !== undefined
          ? Boolean(can_access_shadowing)
          : (existingPermissions?.can_access_shadowing ?? true),
      can_access_cloze:
        can_access_cloze !== undefined
          ? Boolean(can_access_cloze)
          : (existingPermissions?.can_access_cloze ?? true),
      can_access_alignment:
        can_access_alignment !== undefined
          ? Boolean(can_access_alignment)
          : (existingPermissions?.can_access_alignment ?? true),
      can_access_articles:
        can_access_articles !== undefined
          ? Boolean(can_access_articles)
          : (existingPermissions?.can_access_articles ?? true),
      allowed_languages: allowed_languages ||
        existingPermissions?.allowed_languages || ['en', 'ja', 'zh'],
      allowed_levels: allowed_levels || existingPermissions?.allowed_levels || [1, 2, 3, 4, 5],
      max_daily_attempts:
        max_daily_attempts !== undefined
          ? Math.max(0, parseInt(max_daily_attempts) || 50)
          : (existingPermissions?.max_daily_attempts ?? 50),
      model_permissions: model_permissions || existingPermissions?.model_permissions || [],
      api_keys: api_keys || existingPermissions?.api_keys || {},
      ai_enabled:
        ai_enabled !== undefined ? Boolean(ai_enabled) : (existingPermissions?.ai_enabled ?? false),
