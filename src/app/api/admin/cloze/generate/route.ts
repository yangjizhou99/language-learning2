...
function generateBlanksFromPassage(passage: string, lang: 'en'|'ja'|'zh', level: number) {
  ...
  const candidates = words.filter(w => /[A-Za-z\u3040-\u30ff\u4e00-\u9fa5]/.test(w) && !/^\{\{\d+\}\}$/.test(w) && !/^https?:\/\//i.test(w));
...
function extractArrayFromObject(obj: any): any[] | null {
  ...
  const candidateKeys = ['items', 'data', 'result', 'output', 'questions', 'cloze', 'list', 'samples', 'examples', 'records', 'entries', 'item'];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const items = data.map((rawItem: any, index: number) => {
      ...
      if ((!blanksInput || blanksInput.length === 0) && rawItem && typeof rawItem?.blanks === 'object' && !Array.isArray(rawItem?.blanks)) {
        blanksInput = Object.entries(rawItem.blanks).map(([k, v]) => ({ id: Number(k), answer: v }));
      }
      ...
      if ((!blanksInput || blanksInput.length === 0) && rawItem && typeof rawItem?.answers === 'object' && !Array.isArray(rawItem?.answers)) {
        blanksInput = Object.entries(rawItem.answers).map(([k, v]) => ({ id: Number(k), answer: v }));
      }
      ...
      if ((!blanksInput || blanksInput.length === 0) && passage) {
        ...
        for (const re of patterns.slice(0, 5)) {
          const found = Array.from(passage.matchAll(re)).map(m => Number(m[1])).filter(n => Number.isFinite(n));
          ids.push(...found);
        }
        if (ids.length === 0) {
          // 退化为根据下划线段数量生成顺序编号
        ...
        if (ids.length > 0) {
          blanksInput = ids.map((id, i) => ({ id, answer: toStringSafe((rawItem?.answers && (Array.isArray(rawItem.answers) ? rawItem.answers[id - 1] : rawItem.answers?.[id])) || '') }));
        }
      }
    ...
    if (!items.length) {
      ...
      const repair = await chatJSON({
        ...
        model: model!,
        messages: [
          { role: 'system', content: 'You ONLY return a VALID JSON array that matches the required schema for cloze items. No extra text.' },
          { role: 'user', content: `Given the following output, convert it into a JSON array of items with fields: title (string), passage (string with {{1}}, {{2}}...), blanks (array of { id:number, answer:string, type?:"grammar|vocabulary|connector|particle" }). Do not include explanations or alternative answers. Output JSON array only.\n\nOUTPUT TO CONVERT:\n${result.content}` }
        ],
        temperature: 0.2,
        response_json: true
      });
      ...
      if (Array.isArray(repaired)) {
        const items2 = repaired.map((rawItem: any, index: number) => {
          const title = toStringSafe(rawItem?.title) || `Cloze L${level} #${index + 1}`;
          const passage = (
            toStringSafe(rawItem?.passage) ||
            toStringSafe(rawItem?.text) ||
        ...
        if (items2.length) {
          return NextResponse.json({ success: true, items: items2, usage: repair.usage || result.usage });
        }
      }

      // 最后一层兜底：从原始文本强行打捞一个可用 item
      const salvage = salvageItemFromText(result.content, lang, level) || (typeof repair.content === 'string' ? salvageItemFromText(repair.content, lang, level) : null);
      if (salvage) {
        const item = {
          lang,
          level,
          ...
          blanks: salvage.blanks.map(b => ({ id: b.id, answer: toStringSafe(b.answer), acceptable: [], distractors: [], explanation: '', type: validTypes.has(String(b.type).toLowerCase()) ? String(b.type).toLowerCase() : 'vocabulary' })),
          ai_provider: provider,
          ai_model: model!,
          ai_usage: normUsage(repair.usage || result.usage)
        } as const;
        return NextResponse.json({ success: true, items: [item], usage: repair.usage || result.usage });
      }

      console.error('No valid cloze items after repair. Raw (first):', result.content?.slice(0, 500));
      console.error('No valid cloze items after repair. Raw (repair):', (typeof repaired === 'string' ? repaired : repair.content)?.slice(0, 500));
      return NextResponse.json({ error: 'AI 返回数据结构不完整，未解析到有效题目。已尝试自动修复失败，请减少题量（count=1）、降低温度（0.3-0.4）或更换模型/提供商后重试。' }, { status: 502 });
    }
