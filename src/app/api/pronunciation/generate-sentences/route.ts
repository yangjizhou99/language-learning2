// =====================================================
// 使用 DeepSeek 生成更多发音练习句子
// POST /api/pronunciation/generate-sentences
// =====================================================
 ...
 * 获取英语句子长度要求
 */
function getEnglishSentenceLength(level: number): string {
  switch (level) {
    case 1: return '5-8';
    case 2: return '8-12';
    case 3: return '12-15';
 ...
 * 请求体:
 * {
 *   count?: number,  // 生成数量，默认 25
 *   level?: number,  // 难度等级 1-5，默认 2-3
...
要求：
1. 句子长度：8-15个字
2. 难度等级：${targetLevel}（1=最简单，5=最复杂）
3. 覆盖常用汉字和拼音音节
4. 句子要自然、通顺、有实际意义
5. 避免生僻字和专业术语
...
Requirements:
1. Sentence length: ${getEnglishSentenceLength(targetLevel)} words
2. Difficulty level: ${targetLevel} (1=easier, 5=harder)
...
要件：
1. 文の長さ：8-15文字
2. 難易度レベル：${targetLevel}（1=最も簡単、5=最も複雑）
...
文の多様性を確保し、異なるトピックと場面をカバーしてください。`;
    }
    ...
    return NextResponse.json({
      ...
      sentences: inserted || [],
      message: `成功生成 ${inserted?.length || 0} 个句子，自动创建 ${generatedUnits} 条音节关联`,
    });
  } catch (error) {
    console.error('[pronunciation/generate-sentences] 错误:', error);
    return NextResponse.json(
      {
        success: false,
