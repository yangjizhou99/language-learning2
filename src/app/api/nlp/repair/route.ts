...
【重要】词汇/语法分类纠正：
如果你发现某个被标记为"语法"的项目实际上是词汇（名词、接头词、感动词等），请将其纠正为词汇并放入 vocab_entries 而不是 grammar_chunks。
常见被误判的类型：
- 接头词/接尾词作为词汇的一部分（如"展"、"低"、"多"、"近"等）
- 名词后缀（如"済み"、"付け"、"入り"、"買い"等）
- 感动词（如"よいしょ"）
- 助动词或词尾碎片（如"じ"）
- 单位、符号等（如"％"、"か所"）

判断标准：
- 如果该项可以单独作为一个词使用或者是构词成分（词缀），应归类为词汇
- 如果该项是句法结构或功能性表达，应归类为语法
