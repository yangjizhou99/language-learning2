...
function sub(text: string, span?: [number, number]) {
  if (!span) return '';
  return text.slice(span[0], span[1]);
}
function fmtSpan(text: string, span?: [number, number]) {
  return span ? `"${sub(text, span).slice(0, 24)}"` : '—';
}

function paint(
  text: string,
  spans: { span: [number, number]; className: string; title?: string }[],
) {
...
function P1Preview({
  ...
  const spans = items.map((it) => ({
    span: it.span,
    className: it.tag === 'connective' ? 'bg-yellow-200' : 'bg-amber-300',
  }));
  return (
    <div>
...
function ScoreBarP1({
  ...
  res: { prec: number; rec: number; f1: number; tp: number; fp: number; fn: number };
}) {
  return (
    <div className="text-sm text-gray-700">
      Precision {(res.prec * 100) | 0}% · Recall {(res.rec * 100) | 0}% · F1 {(res.f1 * 100) | 0}%
...
function P2Preview({
  ...
  text: string;
  items: { pron: [number, number]; ants: [number, number][] }[];
}) {
  const spans = [
...
function ScoreBarP2({ res }: { res: { acc: number; tp: number; fp: number; fn: number } }) {
  return (
    <div className="text-sm text-gray-700">
      正确率 {(res.acc * 100) | 0}% （TP {res.tp} / FP {res.fp} / FN {res.fn}）
    </div>
  );
}
      ...
      <ul className="space-y-1 text-sm">
        {items.map((t, i) => (
          <li key={i} className="p-2 border rounded">
            ...
            <button onClick={() => onSave(t)} className="ml-2 px-2 py-0.5 border rounded text-xs">
              保存为卡片
            </button>
          </li>
        ))}
...
function ScoreBarP3({ res }: { res: { score: number; detail: number[] } }) {
  return (
    <div className="text-sm text-gray-700">
      匹配度 {(res.score * 100) | 0}% （按 S/V/O 三段计分）
    </div>
  );
}
...
export default function WideReadPage() {
  ...
  const [keys, setKeys] = useState<{ pass1: KeyPass1; pass2: KeyPass2; pass3: KeyPass3 } | null>(
    null,
  );
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      if (since) {
        ...
        const merged: Article[] = Object.values(byId).sort(
          (a: Article, b: Article) =>
            new Date(b.updated_at || b.created_at || 0).getTime() -
            new Date(a.updated_at || a.created_at || 0).getTime(),
        );
        setArticles(merged);
        const maxUpdated: string | null = ((data as Article[]) || []).reduce(
          (m: string | null, d: Article) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (k.error) setErr(k.error.message);
        ...
        setKeys({
          pass1: (k.data?.pass1 || []) as KeyPass1,
          pass2: (k.data?.pass2 || []) as KeyPass2,
          pass3: (k.data?.pass3 || []) as KeyPass3,
        });
  ...
  const loadVoices = async (kind: 'Neural2' | 'WaveNet' | 'all' = 'Neural2') => {
    ...
    const pref =
      j.find((v: Voice) =>
        v.name.startsWith(
          lang === 'zh' ? 'zh-CN-Neural2' : lang === 'ja' ? 'ja-JP-Neural2' : 'en-US-Neural2',
        ),
      ) || j[0];
  ...
  const addP1 = () => {
    ...
    // 去重/不重叠
    setUserP1((prev) =>
      prev.some((x) => overlap(x.span, r)) ? prev : [...prev, { span: r, tag: curTag }],
    );
  ...
  const addP2 = () => {
    ...
    if (modeP2 === 'pron') {
      ...
      setUserP2((prev) =>
        prev.map((x, idx) => (idx === focusedPron ? { ...x, ants: [...x.ants, r] } : x)),
      );
    }
    window.getSelection()?.removeAllRanges();
  };
  ...
  const [userP3, setUserP3] = useState<
    { s: [number, number]; v: [number, number]; o: [number, number] }[]
  >([]);
  const [curTriple, setCurTriple] = useState<
    Partial<{ s: [number, number]; v: [number, number]; o: [number, number] }>
  >({});
  ...
  const cloze = useMemo(
    () => clozeSets.find((s) => s.version === clozeVersion)?.items || [],
    [clozeSets, clozeVersion],
  );
  const [answers, setAnswers] = useState<Record<number, string>>({});
  const norm = (s: string) => (s || '').normalize('NFKC').replace(/\s+/g, ' ').trim().toLowerCase();
  const clozeScore = () => {
    let ok = 0;
    cloze.forEach((it, idx) => {
      if (norm(answers[idx]) === norm(it.answer)) ok++;
    });
  ...
  const submitScores = async () => {
    ...
    const payload = {
      ...
      score: Math.round(
        ((p1.f1 || 0) * 100 + (p2.acc || 0) * 100 + (p3.score || 0) * 100 + (cz.acc || 0) * 100) /
          4,
      ),
    };
    ...
    <main className="max-w-5xl mx-auto p-4 space-y-6">
      ...
      <section className="p-4 bg-white rounded-2xl shadow space-y-3">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <button onClick={fetchList} className="px-3 py-1 rounded border">
            刷新列表
          </button>
          <button onClick={pickRandom} className="px-3 py-1 rounded border">
            随机来一篇
          </button>
        </div>
        ...
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="text-xs text-gray-500">
              来源：
              {cur.source_url ? (
                <a className="underline" href={cur.source_url} target="_blank">
                  原文
                </a>
              ) : (
                '—'
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => loadVoices('Neural2')}
              className="px-2 py-1 border rounded text-sm"
            >
              Neural2 声音
            </button>
            <button
              onClick={() => loadVoices('WaveNet')}
              className="px-2 py-1 border rounded text-sm"
            >
              WaveNet 声音
            </button>
            <select
              onFocus={() => {
                if (!voicesLoaded) loadVoices('Neural2');
              }}
              value={voiceName}
            ...
            >
              {voices.map((v) => (
                <option key={v.name} value={v.name}>
                  {v.name} · {(v.ssmlGender || '').replace('SSML_VOICE_GENDER_', '')}
                </option>
              ))}
            </select>
            ...
            <label className="text-sm flex items-center gap-1">
              ...
              <input
                ...
                max="20"
                value={pitch}
                onChange={(e) => setPitch(Number(e.target.value) || 0)}
                className="w-20 border rounded px-2 py-1"
              />
            </label>
            <button onClick={synth} className="px-3 py-1 rounded bg-black text-white">
              ▶ 合成 TTS
            </button>
          </div>
        ...
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-1">
              <input
                type="radio"
                checked={curTag === 'connective'}
                onChange={() => setCurTag('connective')}
              />
          ...
          <div className="text-xs text-gray-500">
            说明：拖拽选择一段文字后点“标注选区”。系统用
            <span className="px-1 bg-yellow-200 rounded">黄色</span>
            表示你标注的片段；下方“参考答案”来自题库。
          </div>
        </section>
      )}
      ...
      {cur && keys && (
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-1">
              <input type="radio" checked={modeP2 === 'pron'} onChange={() => setModeP2('pron')} />{' '}
              选“代词”
            </label>
            <label className="flex items-center gap-1">
              <input type="radio" checked={modeP2 === 'ant'} onChange={() => setModeP2('ant')} />{' '}
              选“先行词”（会链接到最近一次选的代词）
            </label>
      ...
      {cur && keys && (
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={commitTriple} className="px-3 py-1 rounded bg-black text-white">
              提交一条三元组
            </button>
          </div>
          <div className="text-sm text-gray-600">
            当前：S={fmtSpan(text, curTriple.s)}，V={fmtSpan(text, curTriple.v)}，O=
            {fmtSpan(text, curTriple.o)}
          </div>
          <P3Preview
            text={text}
            items={userP3}
      ...
      {cur && (
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          ...
          <div className="flex items-center gap-2">
            <select
              value={clozeVersion}
              onChange={(e) => setClozeVersion(e.target.value as 'short' | 'long')}
              className="border rounded px-2 py-1"
            >
          ...
          <ClozeView
            ...
            items={cloze}
            answers={answers}
            onChange={(i, v) => setAnswers((p) => ({ ...p, [i]: v }))}
          />
          <div className="text-sm">
            分数：{Math.round(clozeScore().acc * 100)}%（{clozeScore().ok}/{clozeScore().total}）
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => {
                // 保存命中的连接词/搭配为卡片
                cloze.forEach((it) =>
                  saveCard(it.type === 'connective' ? 'connective' : 'collocation', {
                    text: it.answer,
                  }),
                );
              }}
      ...
      {cur && (
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          <div className="flex gap-2">
            <button onClick={submitScores} className="px-3 py-1 rounded bg-emerald-600 text-white">
              提交成绩到 sessions
            </button>
            <button onClick={resetAll} className="px-3 py-1 rounded border">
              重置本篇练习
            </button>
          </div>
        </section>
