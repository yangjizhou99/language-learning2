...
export default function ArticlesAdminList() {
  ...
  const save = async () => {
    if (!editing) return;
    const r = await fetch('/api/admin/articles/items', {
      method: 'PUT',
  ...
  const remove = async (id: string) => {
    const r = await fetch(`/api/admin/articles/items?id=${encodeURIComponent(id)}`, {
      method: 'DELETE',
  ...
  return (
    <main className="max-w-6xl mx-auto p-6 space-y-4">
      ...
      <div>
        <Button asChild>
          <a href="/admin/articles">新增文章 → 生成/录入页</a>
        </Button>
      </div>
      {/* 搜索与筛选 */}
            ...
            .map((it) => (
              <div key={it.id} className="border rounded p-3">
                ...
                <div className="text-xs text-gray-500 mt-1">
                  {it.lang} · {it.genre} · L{it.difficulty}
                </div>
              </div>
            ))}
          ...
          <div className="bg-card text-card-foreground w-full max-w-3xl p-4 rounded border space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditing({ ...editing, difficulty: Number(e.target.value) || 3 })
                }
              />
            </div>
