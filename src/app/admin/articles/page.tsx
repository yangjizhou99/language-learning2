  ...
  return (
    <main className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="flex gap-2">
        <button className={btn(tab === 'fetch')} onClick={() => setTab('fetch')}>
          A 路抓取
        </button>
        <button className={btn(tab === 'manual')} onClick={() => setTab('manual')}>
          手动录入
        </button>
        <button className={btn(tab === 'ai')} onClick={() => setTab('ai')}>
          AI 生成
        </button>
      </div>
...
function FetchForm() {
  ...
  return (
    <section className="p-4 bg-white rounded-2xl shadow space-y-3">
      ...
      <div className="flex gap-2">
        <button onClick={ingest} className="px-3 py-1 rounded bg-black text-white">
          入库
        </button>
      </div>
...
function ManualForm() {
  ...
  return (
    <section className="p-4 bg-white rounded-2xl shadow space-y-3">
      ...
      <Row label="正文（≥200）">
        <textarea
          className="border rounded px-2 py-1 w-full h-56"
          value={text}
      ...
      <div className="flex gap-2">
        <button onClick={submit} className="px-3 py-1 rounded bg-black text-white">
          生成答案键并入库
        </button>
      </div>
      {log && <pre className="p-3 bg-gray-50 rounded text-sm whitespace-pre-wrap">{log}</pre>}
...
function AIForm() {
  ...
  const submit = async () => {
    ...
    setLog(
      r.ok
        ? `草稿已生成：id=${j.draft_id}，标题：${j.title}。请到草稿箱审核发布。`
        : `失败：${j.error}`,
    );
  };
    ...
    <section className="p-4 bg-white rounded-2xl shadow space-y-3">
      ...
      <Row label="目标长度">
        <input
          ...
          value={words}
          onChange={(e) => setWords(Number(e.target.value) || 300)}
        />
      </Row>
      <Row label="模型来源">
        <select
      ...
      <div className="flex gap-2">
        ...
        <Link href="/admin/drafts" className="px-3 py-1 rounded border text-sm">
          查看草稿箱
        </Link>
      </div>
      {log && <pre className="p-3 bg-gray-50 rounded text-sm whitespace-pre-wrap">{log}</pre>}
      <p className="text-xs text-gray-500">
        说明：AI 生成草稿 → 管理员审核 → 发布到正式题库。支持 OpenRouter 多模型选择。
      </p>
    </section>
  );
}
