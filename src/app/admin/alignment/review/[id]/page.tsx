  ...
  async function save() {
    ...
    const r = await fetch(`/api/admin/alignment/drafts/${id}`, {
      ...
      body: JSON.stringify({
        topic: pack.topic,
        tags: pack.tags,
        preferred_style: pack.preferred_style,
        steps: pack.steps,
      }),
    });
    setSaving(false);
      ...
      <div className="bg-white rounded-lg shadow p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <textarea
              className="w-full border rounded px-2 py-1"
              rows={4}
              value={JSON.stringify(pack.preferred_style || {}, null, 2)}
              onChange={(e) => {
                try {
                  setPack({ ...pack, preferred_style: JSON.parse(e.target.value) });
                } catch {}
              }}
            />
          </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={16}
            value={JSON.stringify(pack.steps || {}, null, 2)}
            onChange={(e) => {
              try {
                setPack({ ...pack, steps: JSON.parse(e.target.value) });
              } catch {}
            }}
          />
        </div>
