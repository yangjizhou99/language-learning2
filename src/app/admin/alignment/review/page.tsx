  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <input className="border rounded px-2 py-1" placeholder="搜索主题" value={q} onChange={e=> setQ(e.target.value)} />
        ...
        <div className="bg-white rounded-lg shadow divide-y">
          {list.map(it=> (
            <div key={it.id} className="p-4 flex items-center justify-between">
              <div>
                <div className="text-sm text-gray-500">{it.lang} · {new Date(it.created_at).toLocaleString()}</div>
                <div className="font-medium">{it.topic}</div>
              </div>
              <Link className="px-3 py-1 rounded bg-blue-600 text-white" href={`/admin/alignment/review/${it.id}`}>查看</Link>
            </div>
          ))}
