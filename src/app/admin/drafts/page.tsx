  ...
  return (
    <main className="max-w-4xl mx-auto p-6 space-y-4">
      ...
      <div className="bg-yellow-50 p-3 rounded border text-sm text-yellow-900">
        <strong>调试信息:</strong> loading={String(loading)}, error="{error}", list.length=
        {list.length}, status="{status}"
      </div>
            ...
            list.map((d) => (
              <li key={d.id} className="p-3 border rounded">
                ...
                <div className="text-xs text-gray-500">
                  {d.lang} · {d.genre}/L{d.difficulty} · {new Date(d.created_at).toLocaleString()} ·{' '}
                  {d.source} {d.ai_model ? `· ${d.ai_model}` : ''} · 更新:{' '}
                  {new Date((d as any).updated_at || d.created_at).toLocaleString()}
                </div>
              </li>
            ))
          )}
