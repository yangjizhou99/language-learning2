...
export default function DraftDetail() {
  ...
  useEffect(() => {
    (async () => {
      if (tProvider === 'openrouter') {
        const r = await fetch(`/api/ai/models?provider=${tProvider}`);
        const j = await r.json();
        setTModels(j || []);
        setTModel(j?.[0]?.id || '');
  ...
  const publish = async () => {
    try {
      ...
      if (r.ok) toast.success(`发布成功 (article_id=${j.article_id})`);
      else toast.error(`发布失败：${j.error}`);
    } catch (e: any) {
      setLog(`发布失败: ${e.message || e}`);
    }
  };
    ...
    <main className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <section className="p-4 bg-white rounded-2xl shadow space-y-3">
        ...
        <div className="text-xs text-gray-500">
          {d.lang} · {d.genre}/L{d.difficulty} · 状态：{d.status} · AI:{d.ai_provider || '-'}{' '}
          {d.ai_model || ''}
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => update({ title: d.title })}>
            保存标题
          </Button>
          <Button variant="outline" onClick={() => update({ status: 'approved' })}>
            标记为已审
          </Button>
          <Button variant="outline" onClick={() => update({ status: 'needs_fix' })}>
            需要修改
          </Button>
          <Button variant="outline" onClick={() => update({ status: 'rejected' })}>
            拒绝
          </Button>
          <Dialog>
      ...
      <section className="p-4 bg-white rounded-2xl shadow space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Select
            value={tProvider}
            onValueChange={(v: 'openrouter' | 'deepseek' | 'openai') => setTProvider(v)}
          >
          ...
          <Input
            ...
            className="w-24"
            value={tTemp}
            onChange={(e) => setTTemp(Number(e.target.value) || 0.3)}
          />
          <Button
            onClick={async () => {
              ...
              try {
                ...
                if (!r.ok) {
                  setLog('文本建议失败：' + (j.error || r.statusText));
                  toast.error('文本建议失败：' + (j.error || r.statusText));
                  return;
                }
                setTextSug(j.suggestions);
                setLog(
                  `AI 文本建议已生成。用量：PT=${j.usage.prompt_tokens}, CT=${j.usage.completion_tokens}, TT=${j.usage.total_tokens}`,
                );
                toast.success('AI 文本建议已生成');
              } catch (e: any) {
                setTextGenLoading(false);
                setLog(`文本建议异常：${e.message || e}`);
                toast.error(`文本建议异常：${e.message || e}`);
              }
            }}
          >
        ...
        {textSug && (
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="p-3 bg-gray-50 rounded">
              ...
              <div className="mb-2">
                ...
                <ul className="list-disc pl-5">
                  {(textSug.pass1_suggestions?.connectives || []).map((s: string, i: number) => (
                    <li key={i}>{s}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc pl-5">
                  {(textSug.pass1_suggestions?.time || []).map((s: string, i: number) => (
                    <li key={i}>{s}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="p-3 bg-gray-50 rounded">
              ...
              <ul className="list-disc pl-5">
                {(textSug.pass2_pairs || []).map((p: any, i: number) => (
                  <li key={i}>
                    <b>{p.pronoun}</b> → {p.antecedent_hint}
                  </li>
                ))}
              </ul>
            </div>
            <div className="p-3 bg-gray-50 rounded">
              ...
              <ul className="list-disc pl-5">
                {(textSug.triples_text || []).map((s: string, i: number) => (
                  <li key={i}>{s}</li>
                ))}
              </ul>
            </div>
            <div className="p-3 bg-gray-50 rounded">
              ...
              <ul className="list-disc pl-5">
                {(textSug.cloze_candidates || []).map((c: any, i: number) => (
                  <li key={i}>
                    {c.phrase} <span className="text-xs text-gray-500">({c.hint})</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        {!textSug && (
          <div className="text-xs text-gray-500">
            生成后，这里会显示一份“文字清单”。请参照清单，在下方的可视化标注器中手工框选标注。
          </div>
        )}
      </section>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {genMode === 'ai' && (
            <>
              <Select
                value={provider}
                onValueChange={(v: 'openrouter' | 'deepseek' | 'openai') => setProvider(v)}
              >
                <SelectTrigger className="w-40">
                  <SelectValue placeholder="Provider" />
                </SelectTrigger>
              ...
              <Input
                ...
                className="w-24"
                value={temp}
                onChange={(e) => setTemp(Number(e.target.value) || 0.3)}
              />
            </>
          )}
          <Button
            onClick={async () => {
              ...
              try {
                ...
                const r = await fetch(`/api/admin/drafts/${id}/suggest-keys`, {
                  ...
                  body: JSON.stringify(
                    genMode === 'ai'
                      ? { mode: 'ai', provider, model, temperature: temp }
                      : { mode: 'rule' },
                  ),
                });
                const j = await r.json();
                setGenLoading(false);
                if (!r.ok) {
                  setLog('生成失败：' + (j.error || r.statusText));
                  toast.error('生成失败：' + (j.error || r.statusText));
                  return;
                }
                setSug(j);
                setLog(
                  genMode === 'ai'
                    ? `AI 用量：PT=${j.usage.prompt_tokens}, CT=${j.usage.completion_tokens}, TT=${j.usage.total_tokens}`
                    : '规则生成完成',
                );
                toast.success(genMode === 'ai' ? 'AI 建议生成完成' : '规则生成完成');
              } catch (e: any) {
                setGenLoading(false);
                setLog(`生成异常：${e.message || e}`);
                toast.error(`生成异常：${e.message || e}`);
              }
            }}
          >
        ...
        {sug && (
          <div className="p-3 bg-gray-50 rounded">
            <div className="text-sm mb-2">
              建议摘要：P1={sug.suggestion.pass1.length} · P2={sug.suggestion.pass2.length} · P3=
              {sug.suggestion.pass3.length} · ClozeS={sug.suggestion.cloze_short.length} · ClozeL=
              {sug.suggestion.cloze_long.length}
            </div>
            <div className="flex gap-2">
              <Button
                variant="outline"
              ...
              <Button variant="outline" onClick={() => setSug(null)}>
                清除建议
              </Button>
            </div>
      ...
      <Annotator
        ...
        onValidateSave={async (payload) => {
          try {
            ...
            if (!r.ok) {
              setSaving(false);
              setLog('校验失败：' + (j.error || r.statusText));
              return;
            }
            setP1(j.keys.pass1);
            setP2(j.keys.pass2);
            ...
            await update({
              keys: j.keys,
              cloze_short: j.cloze_short,
              cloze_long: j.cloze_long,
              validator_report: j.report,
            });
            setSaving(false);
            setLog(
              `已保存。摘要：P1=${j.report.pass1}, P2=${j.report.pass2}, P3=${j.report.pass3}, ClozeS=${j.report.cloze_short}, ClozeL=${j.report.cloze_long}`,
            );
          } catch (e: any) {
            setSaving(false);
            setLog(`校验异常：${e.message || e}`);
          }
        }}
      />
      ...
      <section className="p-4 bg-white rounded-2xl shadow space-y-2">
        ...
        />
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => update({ text: d.text })}>
            保存正文
          </Button>
        </div>
      </section>
...
function Annotator({
  ...
  text: string;
  p1: KeyP1[];
  setP1: Dispatch<SetStateAction<KeyP1[]>>;
  p2: KeyP2[];
  setP2: Dispatch<SetStateAction<KeyP2[]>>;
  p3: KeyP3[];
  setP3: Dispatch<SetStateAction<KeyP3[]>>;
  czShort: Cloze[];
  setCzShort: Dispatch<SetStateAction<Cloze[]>>;
  czLong: Cloze[];
  setCzLong: Dispatch<SetStateAction<Cloze[]>>;
  ...
  function getSelectionSpan(): Span | null {
    ...
    const startEl = (rng.startContainer.parentElement ??
      (rng.startContainer as any)) as HTMLElement;
    const endEl = (rng.endContainer.parentElement ?? (rng.endContainer as any)) as HTMLElement;
    const sAttr = (startEl as any)?.dataset?.idx,
      eAttr = (endEl as any)?.dataset?.idx;
  ...
  const charStyles = useMemo(() => {
    ...
    if (tab === 'p1') {
      highlights.push(
        ...p1.map((k) => ({
          span: k.span,
          className: k.tag === 'connective' ? 'bg-yellow-200' : 'bg-amber-200',
          title: k.tag,
        })),
      );
    } else if (tab === 'p2') {
      highlights.push(
        ...p2.map((k) => ({ span: k.pron, className: 'bg-blue-200', title: 'pronoun' })),
      );
      p2.forEach((k) =>
        k.antecedents.forEach((a) =>
          highlights.push({ span: a, className: 'bg-green-200', title: 'antecedent' }),
        ),
      );
    } else if (tab === 'p3') {
      highlights.push(
    ...
    const styles: { className?: string; title?: string }[] = Array(text.length)
      .fill(null)
      .map(() => ({}));
  ...
  // 操作：添加/删除
  const addP1 = () => {
    const r = getSelectionSpan();
    if (!r) return;
    setP1((prev) =>
      prev.some((x) => !(r[1] <= x.span[0] || x.span[1] <= r[0]))
        ? prev
        : [...prev, { span: r, tag: p1Tag }],
    );
  ...
  const addP2 = () => {
    ...
    if (p2Mode === 'pron') {
      ...
      setP2((prev) =>
        prev.map((x, i) => (i === focusedPron ? { ...x, antecedents: [...x.antecedents, r] } : x)),
      );
    }
    window.getSelection()?.removeAllRanges();
  };
  ...
  const addCloze = () => {
    ...
    if (czVer === 'short') setCzShort((prev) => [...prev, item]);
    else setCzLong((prev) => [...prev, item]);
    window.getSelection()?.removeAllRanges();
  };
  const delCloze = (i: number) =>
    czVer === 'short'
      ? setCzShort((prev) => prev.filter((_, idx) => idx !== i))
      : setCzLong((prev) => prev.filter((_, idx) => idx !== i));
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {tab === 'p1' && (
          <>
            ...
            <select
              value={p1Tag}
              onChange={(e) => setP1Tag(e.target.value as any)}
              className="border rounded px-2 py-1"
            >
              <option value="connective">连接词</option>
              <option value="time">时间表达</option>
            </select>
            <button onClick={addP1} className="px-3 py-1 rounded border">
              添加选区
            </button>
          </>
        )}
        ...
        {tab === 'p2' && (
          <>
            ...
            <button onClick={addP2} className="px-3 py-1 rounded border">
              添加
            </button>
          </>
        )}
        ...
        {tab === 'p3' && (
          <>
            <button onClick={() => addSeg('s')} className="px-3 py-1 rounded border">
              选择 S
            </button>
            <button onClick={() => addSeg('v')} className="px-3 py-1 rounded border">
              选择 V
            </button>
            <button onClick={() => addSeg('o')} className="px-3 py-1 rounded border">
              选择 O
            </button>
            <button onClick={commitP3} className="px-3 py-1 rounded bg-black text-white">
              提交一条
            </button>
          </>
        )}
        ...
        {tab === 'cloze' && (
          <>
            ...
            <select
              value={czVer}
              onChange={(e) => setCzVer(e.target.value as any)}
              className="border rounded px-2 py-1"
            >
              <option value="short">短版</option>
              <option value="long">长版</option>
            </select>
            <button onClick={addCloze} className="px-3 py-1 rounded border">
              将选区设为空格
            </button>
          </>
        )}
      ...
      <div className="p-3 bg-gray-50 rounded leading-7 text-[15px]">
        {chars.map((ch, i) => (
          <span
            key={i}
            data-idx={i}
            className={charStyles[i]?.className}
            title={charStyles[i]?.title}
          >
            {ch}
          </span>
        ))}
            ...
            <ul className="text-sm space-y-1">
              {p1.map((it, i) => (
                <li key={i} className="p-2 border rounded">
                  {it.tag} · [{it.span[0]}, {it.span[1]}] · “
                  {text.slice(it.span[0], it.span[1]).slice(0, 24)}”
                  <button
                    onClick={() => delP1(i)}
        ...
        {tab === 'p3' && (
          <div>
            ...
            <ul className="text-sm space-y-1">
              {p3.map((t, i) => (
                <li key={i} className="p-2 border rounded">
                  <b>S</b>“{text.slice(t.s[0], t.s[1])}” · <b>V</b>“{text.slice(t.v[0], t.v[1])}” ·{' '}
                  <b>O</b>“{text.slice(t.o[0], t.o[1])}”
                  <button
                    onClick={() => delP3(i)}
                    className="ml-2 px-2 py-0.5 text-xs border rounded"
                  >
            ...
            <div className="text-xs text-gray-500 mt-1">
              当前未提交：S=
              {'s' in (curTriple as any) && (curTriple as any).s
                ? `[${(curTriple as any).s[0]},${(curTriple as any).s[1]}]`
                : '—'}{' '}
              / V=
              {'v' in (curTriple as any) && (curTriple as any).v
                ? `[${(curTriple as any).v[0]},${(curTriple as any).v[1]}]`
                : '—'}{' '}
              / O=
              {'o' in (curTriple as any) && (curTriple as any).o
                ? `[${(curTriple as any).o[0]},${(curTriple as any).o[1]}]`
                : '—'}
            </div>
          </div>
        )}
        {tab === 'cloze' && (
          <div>
            <div className="text-sm font-medium mb-1">
              Cloze 空格（{czVer === 'short' ? '短版' : '长版'}）
            </div>
            <ul className="text-sm space-y-1">
              {(czVer === 'short' ? czShort : czLong).map((it, i) => (
                <li key={i} className="p-2 border rounded">
                  [{it.start},{it.end}] “{text.slice(it.start, it.end).slice(0, 36)}” ·{' '}
                  {it.hint || 'blank'}
                  <button
                    onClick={() => delCloze(i)}
                    className="ml-2 px-2 py-0.5 text-xs border rounded"
