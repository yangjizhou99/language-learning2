...
export default function BatchPage() {
  ...
  async function refresh() {
    ...
    const token = session?.access_token;
    const r = await fetch(`/api/admin/drafts/batch/status?id=${batchId}`, {
    ...
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <section className="p-4 bg-white rounded-2xl shadow space-y-3">
        ...
        <div className="flex gap-2">
          ...
          {batchId && (
            <Button variant="outline" onClick={() => runOnce(5)} disabled={running}>
              {running ? '运行中…' : '手动执行 5 条'}
            </Button>
          )}
        </div>
      ...
      {status && (
        <section className="p-4 bg-white rounded-2xl shadow space-y-3">
          ...
          <div className="text-sm">
            模型：{status.batch.provider} / {status.batch.model} · {status.batch.lang} ·{' '}
            {status.batch.genre}
          </div>
          <div className="text-sm">
            总用量：PT={status.batch.totals.prompt_tokens} · CT=
            {status.batch.totals.completion_tokens} · TT={status.batch.totals.total_tokens}
          </div>
          <div className="text-sm">
            计数：pending {status.counts.pending} · processing {status.counts.processing} · done{' '}
            {status.counts.done} · failed {status.counts.failed}
          </div>
          <details>
            <summary className="cursor-pointer text-sm">按难度汇总</summary>
            <pre className="text-xs bg-gray-50 p-2 rounded overflow-auto">
              {JSON.stringify(status.perDifficulty, null, 2)}
            </pre>
          </details>
          <details>
