  ...
  const publishSelectedDrafts = async (selectedItems: QuestionItem[], currentRunId?: number) => {
    ...
    try {
      ...
      if (currentRunId && currentRunId === runIdRef.current) {
        ...
        console.warn('发布完成但请求已过期，跳过日志输出', {
          currentRunId,
          currentRunIdRef: runIdRef.current,
        });
      } else {
        console.log(`成功发布 ${publishedCount}/${selectedItems.length} 个草稿`);
      }
      ...
      const response = await fetch('/api/admin/question-bank/specialized-sync', {
        ...
        body: JSON.stringify({
          sourceConfig: {
            url:
              sourceConfig.url ||
              `postgresql://${sourceConfig.username}:${sourceConfig.password}@${sourceConfig.host}:${sourceConfig.port}/${sourceConfig.database}?sslmode=${sourceConfig.ssl ? 'require' : 'disable'}`,
          },
          targetConfig: {
            url:
              targetConfig.url ||
              `postgresql://${targetConfig.username}:${targetConfig.password}@${targetConfig.host}:${targetConfig.port}/${targetConfig.database}?sslmode=${targetConfig.ssl ? 'require' : 'disable'}`,
          },
          supabaseConfig,
      ...
      if (response.ok) {
        ...
        if (hasErrors) {
          ...
          setPackingProgress((prev) => ({
            ...
            itemsProcessed:
              data.results?.reduce((sum: number, result: any) => sum + result.itemsCount, 0) || 0,
            filesProcessed:
              data.results?.reduce((sum: number, result: any) => sum + result.filesCount, 0) || 0,
          }));
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
              ...
              <div className="text-xs text-purple-600 mt-1">用于处理音频文件和翻译数据的存储</div>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">
                  L1:{stats.byLevel.L1} L2:{stats.byLevel.L2} L3:{stats.byLevel.L3} L4:
                  {stats.byLevel.L4} L5:{stats.byLevel.L5}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-2">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                共 {totalItems} 个草稿题目，第 {currentPage} / {totalPages} 页
              </CardDescription>
            </div>
      ...
      {isPacking && (
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>正在处理您的打包请求，请稍候...</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {/* 总体进度 */}
              <div className="space-y-2">
                ...
                <div className="text-xs text-gray-500">{packingProgress.details}</div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-600">已处理题目</div>
                  <div className="text-xs text-gray-400">/ {packingProgress.itemsTotal}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {packingProgress.filesProcessed}
                  </div>
                  <div className="text-xs text-gray-600">已处理文件</div>
                  <div className="text-xs text-gray-400">/ {packingProgress.filesTotal}</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {packingProgress.current}
                  </div>
                  <div className="text-xs text-gray-600">当前步骤</div>
                  <div className="text-xs text-gray-400">/ {packingProgress.total}</div>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-600">已发布草稿</div>
                  <div className="text-xs text-gray-400">/ {packingProgress.publishTotal}</div>
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      name: '准备中...',
                      status:
                        packingProgress.current >= 1
                          ? 'completed'
                          : packingProgress.current === 0
                            ? 'current'
                            : 'pending',
                    },
                    {
                    ...
                    <div key={step.step} className="flex items-center gap-2 text-xs">
                      ...
                      <span
                        className={
                          step.status === 'current' ? 'font-medium text-blue-600' : 'text-gray-600'
                        }
                      >
                        {step.name}
                      </span>
      ...
      {packingResults.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>数据同步完成，系统已自动处理所有冲突情况</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {result.errors.length > 0 && (
                        <p className="text-sm text-red-600 mt-1">{result.errors.join(', ')}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-gray-600">{result.itemsCount} 个题目</span>
                    <span className="text-sm text-gray-600">{result.filesCount} 个文件</span>
                    <span className="text-sm text-gray-600">{result.themesCount} 个主题</span>
                    <span className="text-sm text-gray-600">{result.subtopicsCount} 个子主题</span>
                    <span className="text-sm text-gray-600">
                      {result.publishedCount || 0} 个已发布
                    </span>
                    <span className="text-sm text-gray-600">{result.duration}ms</span>
                    {result.errors.length > 0 && (
                      <span className="text-sm text-red-600">{result.errors.length} 个错误</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                <div>
                  • <strong>相同题目ID：</strong>自动更新现有题目内容，保持ID不变
                </div>
                <div>
                  • <strong>相同主题ID：</strong>更新主题信息，保持关联关系
                </div>
                <div>
                  • <strong>相同音频文件：</strong>覆盖现有音频文件，更新URL
                </div>
                <div>
                  • <strong>翻译数据：</strong>用新翻译覆盖旧翻译内容
                </div>
                <div>
                  • <strong>草稿同步：</strong>草稿保持草稿状态同步到远程数据库
                </div>
                <div>
                  • <strong>本地发布：</strong>将草稿移动到本地正式题目表，删除草稿记录
                </div>
                <div>
                  • <strong>事务安全：</strong>所有操作在事务中执行，确保数据一致性
                </div>
              </div>
            </div>
          </CardContent>
