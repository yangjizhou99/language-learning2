    ...
    <div className="container mx-auto p-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="flex items-end">
              <Button onClick={loadAllData} variant="outline" className="w-full" disabled={loading}>
                {loading ? '加载中...' : '刷新数据'}
              </Button>
            </div>
          </div>
            ...
            <div className="flex gap-2">
              ...
              <Dialog open={showCreatePackage} onOpenChange={setShowCreatePackage}>
                <DialogTrigger asChild>
                  <Button disabled={selectedCount === 0} className="bg-blue-600 hover:bg-blue-700">
                ...
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription>将选中的 {selectedCount} 个题目打包导出</DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="flex justify-end gap-2 mt-6">
                    ...
                    <Button onClick={createExportPackage}>创建导出包</Button>
                  </div>
                </DialogContent>
              </Dialog>
            </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>共 {filteredItems.length} 个题目</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      {exportPackages.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>已创建的导出包，可以下载或同步到远程数据库</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-sm text-gray-500">共 {stats.total} 个题目</span>
                        <div className="flex gap-2">
                          {Object.entries(stats.byType).map(([type, count]) => (
                            <Badge key={type} variant="outline" className="text-xs">
                              {type === 'shadowing' ? '跟读' : type === 'cloze' ? '完形' : '对齐'}:{' '}
                              {count}
                            </Badge>
                          ))}
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => exportToJSON(pkg)}>
