      ...
      <nav className="bg-background border-b">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-6">
            <Link href="/" className="text-lg font-semibold">Lang Trainer</Link>
            <div className="flex items-center space-x-4">
              <Link href="/admin" className="hover:underline">控制台</Link>
              <Link href="/admin/cloze/ai" className="hover:underline">Cloze 管理</Link>
              <Link href="/admin/cloze/drafts" className="text-primary font-medium">Cloze 草稿箱</Link>
              <Link href="/admin/setup" className="hover:underline">权限设置</Link>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <Button asChild variant="outline" size="sm"><Link href="/admin">返回控制台</Link></Button>
      ...
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <div className="rounded-lg border bg-card text-card-foreground p-4 mb-4 grid grid-cols-2 md:grid-cols-6 gap-3">
          ...
          <div>
            ...
            <Select value={String(level)} onValueChange={(v)=> setLevel(v==='all'?'all':parseInt(v))}>
              <SelectTrigger><SelectValue placeholder="全部" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">全部</SelectItem>
                {[1,2,3,4,5].map(l=> <SelectItem key={l} value={String(l)}>L{l}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <Input placeholder="标题/主题/内容" value={query} onChange={e=>setQuery(e.target.value)} />
          </div>
        </div>
                ...
                <tr key={d.id} className="border-t">
                  ...
                  <td className="p-3 space-x-2">
                    ...
                    {d.status !== 'approved' && (
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button variant="link" size="sm">发布</Button>
                        </DialogTrigger>
                        <DialogContent>
                          <DialogHeader>
                          ...
                          <div className="mt-4 flex justify-end gap-2">
                            ...
                            <DialogClose asChild>
                              <Button onClick={async ()=>{
                                ...
                                if (res.ok) { toast.success('发布成功'); location.reload(); } else { const t = await res.text(); toast.error('发布失败: ' + t); }
                              }}>确认发布</Button>
                            </DialogClose>
                          </div>
                        </DialogContent>
