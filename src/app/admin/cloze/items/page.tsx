...
export default function ClozeItemsAdmin() {
  ...
  return (
    <main className="max-w-6xl mx-auto p-6 space-y-4">
      ...
      <div>
        <Button asChild><a href="/admin/cloze/ai">新增题目 → 生成页</a></Button>
      </div>
      {/* 搜索与筛选 */}
      <div className="flex flex-wrap gap-2 items-center">
        <Input placeholder="搜索标题/主题" value={q} onChange={e=>setQ(e.target.value)} className="w-64" />
        ...
        <div className="grid gap-3">
          {items
            ...
            <div key={it.id} className="border rounded p-3 bg-card text-card-foreground">
              ...
              <div className="text-xs text-muted-foreground mt-1">{it.lang} · L{it.level} · {it.topic}</div>
            </div>
          ))}
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-card text-card-foreground w-full max-w-3xl p-4 rounded border space-y-3">
            ...
            <div className="text-xs text-muted-foreground">暂支持整体编辑；如需分 blank 编辑，可在 Cloze 生成页处理。</div>
