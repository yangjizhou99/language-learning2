...
const AudioPlayer = memo<AudioPlayerProps>(
  ({ src, title = '音频', duration, className = '', onPlay, onPause, onEnded, onError }) => {
    ...
    return (
      <div className={`bg-blue-50 border border-blue-200 rounded p-3 ${className}`}>
        <div className="flex items-center gap-3 mb-2">
          ...
          {duration && (
            <span className="text-xs text-blue-600">时长: {Math.round(duration / 1000)}秒</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleMuteToggle} className="text-blue-600 hover:text-blue-700">
              {volumeIcon}
            </button>
            <input
              type="range"
