...
const ClozeRenderer = memo<ClozeRendererProps>(
  ({ item, answers, showAnswers = false, onAnswerChange, onSubmit }) => {
    // 记忆化正则表达式
    const blankRegex = useMemo(() => /(\{\{(\d+)\}\})/g, []);

    // 记忆化输入框组件
    const InputField = useMemo(() => {
      return memo<{ id: number; value: string; placeholder: string }>(
    ...
    const UserAnswerDisplay = useMemo(() => {
      return memo<{ id: number; value: string }>(({ id, value }) => (
        <span className="mx-1 px-2 py-1 bg-blue-100 text-blue-800 border border-blue-300 rounded">
          {value || '(未填写)'}
        </span>
      ));
    }, []);
    ...
    const CorrectAnswerDisplay = useMemo(() => {
      return memo<{ id: number; answer: string }>(({ id, answer }) => (
        <span className="mx-1 px-2 py-1 bg-green-100 text-green-800 border border-green-300 rounded">
          {answer}
        </span>
      ));
    }, []);
    ...
    const renderCloze = useMemo(() => {
      ...
      if (lastIndex < text.length) {
        parts.push(<span key="t-tail">{text.slice(lastIndex)}</span>);
      }
    ...
    return <div className="leading-8 text-lg">{renderCloze}</div>;
  },
);
