  ...
  for (const bucketName of buckets) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i += CONCURRENT_LIMIT) {
        ...
        console.log(
          `🔄 处理批次 ${Math.floor(i / CONCURRENT_LIMIT) + 1}/${Math.ceil(files.length / CONCURRENT_LIMIT)} (${batch.length} 个文件)`,
        );
        ...
        console.log(
          `📊 进度: ${processedCount}/${files.length} (${Math.round((processedCount / files.length) * 100)}%)`,
        );
