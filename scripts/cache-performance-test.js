...
async function testCacheEffect() {
  ...
  for (const testCase of testCases) {
    ...
    if (times.length > 0) {
      ...
      console.log(
        `   🔄 缓存效果: ${cacheImprovement > 0 ? '✅' : '❌'} ${Math.abs(cacheImprovement).toFixed(1)}% ${cacheImprovement > 0 ? '提升' : '下降'}`,
      );
      if (errors.length > 0) {
...
function generateCacheReport(apiResults, dbResults) {
  ...
  const report = {
    ...
    summary: {
      ...
      avgCacheImprovement:
        apiResults.reduce((sum, r) => sum + r.cacheImprovement, 0) / apiResults.length,
      bestCacheImprovement: apiResults.reduce((max, r) =>
        r.cacheImprovement > max.cacheImprovement ? r : max,
      ),
      worstCacheImprovement: apiResults.reduce((min, r) =>
        r.cacheImprovement < min.cacheImprovement ? r : min,
      ),
    },
  };
  ...
  if (apiResults.length > 0) {
    ...
    apiResults.forEach((result, index) => {
      ...
      console.log(
        `   缓存效果: ${result.cacheImprovement > 0 ? '✅' : '❌'} ${Math.abs(result.cacheImprovement).toFixed(1)}% ${result.cacheImprovement > 0 ? '提升' : '下降'}`,
      );
  ...
  if (apiResults.length > 0) {
    ...
    console.log(
      `最佳缓存效果: ${report.summary.bestCacheImprovement.name} (${report.summary.bestCacheImprovement.cacheImprovement.toFixed(1)}%)`,
    );
  }
  if (dbResults.length > 0) {
