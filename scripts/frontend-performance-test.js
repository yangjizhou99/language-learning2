...
async function testPage(browser, testCase) {
  ...
  if (results.length > 0) {
    ...
    const avgInteractionTime =
      results.reduce((sum, r) => sum + r.interactionTime, 0) / results.length;
    const avgFirstPaint =
      results.reduce((sum, r) => sum + r.metrics.paint.firstPaint, 0) / results.length;
    ...
    return {
      ...
      avgFirstPaint,
      avgDomContentLoaded:
        results.reduce((sum, r) => sum + r.metrics.navigation.domContentLoaded, 0) / results.length,
      avgFirstContentfulPaint:
        results.reduce((sum, r) => sum + r.metrics.paint.firstContentfulPaint, 0) / results.length,
    };
  }
...
function generateFrontendReport(results) {
  ...
  const report = {
    ...
    summary: {
      ...
      avgInteractionTime:
        results.reduce((sum, r) => sum + r.avgInteractionTime, 0) / results.length,
  ...
  console.log(
    `最快页面: ${report.summary.fastestPage.name} (${report.summary.fastestPage.avgLoadTime.toFixed(2)}ms)`,
  );
  console.log(
    `最慢页面: ${report.summary.slowestPage.name} (${report.summary.slowestPage.avgLoadTime.toFixed(2)}ms)`,
  );
  console.log('');
