...
function generateCombinedReport(dbReport, apiReport, frontendReport) {
  ...
  const combinedReport = {
    ...
    summary: {
      totalTests: (dbReport?.summary?.totalTests || 0) + 
  ...
  if (dbReport) {
    ...
    console.log(`   最快查询: ${dbReport.summary.fastestTest.name} (${dbReport.summary.fastestTest.avgTime.toFixed(2)}ms)`);
    console.log(`   最慢查询: ${dbReport.summary.slowestTest.name} (${dbReport.summary.slowestTest.avgTime.toFixed(2)}ms)`);
    console.log('');
  }
  ...
  if (apiReport) {
    ...
    console.log(`   最快 API: ${apiReport.summary.fastestApi.name} (${apiReport.summary.fastestApi.avgTime.toFixed(2)}ms)`);
    console.log(`   最慢 API: ${apiReport.summary.slowestApi.name} (${apiReport.summary.slowestApi.avgTime.toFixed(2)}ms)`);
  ...
  if (frontendReport) {
    ...
    console.log(`   最快页面: ${frontendReport.summary.fastestPage.name} (${frontendReport.summary.fastestPage.avgLoadTime.toFixed(2)}ms)`);
    console.log(`   最慢页面: ${frontendReport.summary.slowestPage.name} (${frontendReport.summary.slowestPage.avgLoadTime.toFixed(2)}ms)`);
    console.log('');
  }
...
function generateRecommendations(dbReport, apiReport, frontendReport) {
  ...
  if (dbReport) {
    ...
    if (dbReport.summary.slowestTest.avgTime > 500) {
      recommendations.push(`最慢查询 "${dbReport.summary.slowestTest.name}" 需要优化，考虑添加更多索引`);
    }
  }
