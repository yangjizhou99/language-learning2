...
// 测试单个表的完整同步（使用修复后的逻辑）
async function testTableSyncFixed(localClient, prodClient, tableName) {
  try {
    log(`\n🔧 测试表 ${tableName} 的完整同步（修复版）`, 'blue');
...
async function main() {
  ...
  try {
    ...
    for (const tableName of problemTables) {
      const result = await testTableSyncFixed(localClient, prodClient, tableName);
    ...
    if (successCount === results.length) {
      ...
      log('🌐 访问: http://localhost:3000/admin/database-sync', 'cyan');
    } else {
      log('\n⚠️ 部分表同步失败', 'yellow');
